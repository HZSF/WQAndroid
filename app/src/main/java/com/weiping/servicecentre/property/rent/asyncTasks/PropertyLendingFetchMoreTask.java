package com.weiping.servicecentre.property.rent.asyncTasks;
...
public class PropertyLendingFetchMoreTask extends AsyncTask<String, Void, String> {
    ...
    public PropertyLendingFetchMoreTask(Activity mActivity, ListView listView, PropertyLendingListAdapter adapter, View moreView, int startId) {
        super();
                ...
                if (jsonData.has("error") && Constants.UNAUTHORIZED.equalsIgnoreCase((String) jsonData.get("error"))) {
                    ...
                    if(jsonItems != null && jsonItems.length() > 0){
                        for(int i=0; i<jsonItems.length(); i++){
                            ...
                            items.add(new PropertyLendingListItem(id, address_area, category_name, area, levels, ask_price, description, submit_time));
                        }
